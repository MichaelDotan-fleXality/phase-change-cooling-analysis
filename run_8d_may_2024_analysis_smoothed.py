"""
8-Day Potential Analysis (May 1-8, 2024) - Smoothed Schedule Version
Runs a full potential analysis for 8 days (May 1-8, 2024).
Uses smoothed_price_schedule for less dynamic behavior.
"""
...
# Use smoothed schedule for less dynamic behavior
SCHEDULE_TEMP_TYPE = "smoothed_price_schedule"

print("=" * 70)
print("8-Day Potential Analysis (May 1-8, 2024) - SMOOTHED SCHEDULE VERSION")
print("=" * 70)
print(f"Schedule Type: {SCHEDULE_TEMP_TYPE}")
...
# Analysis period: 8 days (May 1-8, 2024)
start_date = pd.Timestamp("2024-05-01 00:00:00")
end_date = pd.Timestamp("2024-05-08 23:59:59")

# PV location (from config)
pv_location_lat = PV_LOCATION_LAT
pv_location_lon = PV_LOCATION_LON

# Report directory (different from original)
report_directory = "reports/bako/8d_analysis_may_2024_smoothed"
...
try:
    ...
    # Filter for May 1-8, 2024 (use query to avoid KeyError)
    mask = (df_power.index >= start_date) & (df_power.index <= end_date)
...
try:
    if os.path.exists(cams_path):
        ...
        # Filter for May 1-8, 2024
        mask = (solar_data.index >= start_date) & (solar_data.index <= end_date)
...
if solar_data is not None:
    try:
        ...
        print(f"   [OK] Total PV energy for period: {total_pv_energy:.2f} kWh")
    except Exception as e:
...
try:
    ...
    print(f"   [INFO] Fetching prices from API for May 1-8, 2024...")
    ...
    # Use simulated prices with daily variation
    hours = df_power.index.hour + df_power.index.minute / 60
    daily = 30 * np.sin(2 * np.pi * (hours - 6) / 24)
    base_price = 80
    df_power["Spot Market Price (€/MWh)"] = base_price + daily
...
# =============================================================================
# 6. RUN ANALYSIS (SEPARATE SYSTEM OPTIMIZATION)
# =============================================================================
print("\n6. Running potential analysis with separate system optimization...")
print("=" * 70)
print(f"   Schedule Type: {SCHEDULE_TEMP_TYPE} (Smoothed)")
...
optimize_separate_systems(
    ...
    cop=COP,
    schedule_temp_type="smoothed_price_schedule",  # Use smoothed schedule
    cooling_systems=cooling_systems,
...
print("\n" + "=" * 70)
print("8-Day Analysis Complete! (Smoothed Schedule Version)")
print("=" * 70)
